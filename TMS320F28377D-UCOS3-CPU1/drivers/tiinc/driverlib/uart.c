...
{
    return((ui32Base == SCIA_BASE) || (ui32Base == UARTB_BASE) ||
...
UARTRXIntRegister(uint32_t ui32Base, void (*pfnHandler)(void))
{
    ...
    //
    ui32Int = ((ui32Base == SCIA_BASE) ? INT_SCIA_RX : INT_SCIB_RX );
...
UARTTXIntRegister(uint32_t ui32Base, void (*pfnHandler)(void))
{
    ...
    // Determine the interrupt number based on the UART port.
    //
    ui32Int = ((ui32Base == SCIA_BASE) ? INT_SCIA_TX : INT_SCIB_TX );
...
//*****************************************************************************
void
UARTInterrupt_disable(uint32_t ui32Base, uint32_t ui32IntFlags)
{
    //
    // Check the arguments.
    //
