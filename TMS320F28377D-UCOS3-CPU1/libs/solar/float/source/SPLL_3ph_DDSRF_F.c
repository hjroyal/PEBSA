...
void SPLL_3ph_DDSRF_F_FUNC(SPLL_3ph_DDSRF_F *spll_obj) {
    ...
    // Decoupling Network      //
    //-------------------------//
    spll_obj->d_p_decoupl = spll_obj->d_p - (spll_obj->d_n_decoupl_lpf * spll_obj->cos_2theta)
                            - (spll_obj->q_n_decoupl * spll_obj->sin_2theta);
    spll_obj->q_p_decoupl = spll_obj->q_p + (spll_obj->d_n_decoupl_lpf * spll_obj->sin_2theta)
                            - (spll_obj->q_n_decoupl * spll_obj->cos_2theta);

    spll_obj->d_n_decoupl = spll_obj->d_n - (spll_obj->d_p_decoupl_lpf * spll_obj->cos_2theta)
                            + (spll_obj->q_p_decoupl * spll_obj->sin_2theta);
    spll_obj->q_n_decoupl = spll_obj->q_n - (spll_obj->d_p_decoupl_lpf * spll_obj->sin_2theta)
    ...
    // Loop Filter                     //
    //---------------------------------//
    spll_obj->ylf[0] = spll_obj->ylf[1] + (spll_obj->lpf_coeff.B0_lf * spll_obj->v_q[0])
                       + (spll_obj->lpf_coeff.B1_lf * spll_obj->v_q[1]);
