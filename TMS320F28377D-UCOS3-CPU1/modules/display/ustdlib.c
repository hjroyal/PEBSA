...
char* ctohex(char* Bcd,uint16_t Bin)
{
   uint16_t i;
      Bcd[0]=Bin/16;
      Bcd[1]=Bin%16;
...
//*****************************************************************************
int
uvsnprintf(char * restrict s, size_t n, const char * restrict format, va_list arg)
{
...
again:
            ...
            switch(*format++)
            {
                ...
                // convierte a hexa una cadena de caracteres
                case 'H':
                {
                    // Get the string pointer from the varargs.
                    pcStr = va_arg(arg, char *);
                    uint16_t len=va_arg(arg, uint16_t);
...
convert:
                    ...
                    {
                        // Copy the character to the output buffer if there is room.
                        if(n != 0)
                        {
